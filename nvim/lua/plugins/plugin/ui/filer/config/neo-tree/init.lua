...
function M.on_attach(bufnr)
  ...
  -- home
  local goto_home = function()
    api.tree.change_root("~")
  end
  keymap.set("n", "~", goto_home, opts("Change Directory to HOME"))
  -- setting
  local goto_config = function()
    api.tree.change_root(g.my_initvim_path)
  end
  keymap.set("n", "^", goto_config, opts("Rename"))
  -- repository
  local goto_repos = function()
    api.tree.change_root("~" .. env.get_path_splitter_for_current_env() .. "repos")
  end
  keymap.set("n", "\\", goto_repos, opts("Rename"))
  -- documents
  local goto_doc = function()
    api.tree.change_root("~" .. env.get_path_splitter_for_current_env() .. "Documents")
  end
